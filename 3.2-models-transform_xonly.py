# ##############
# Use for interpreting linear regression: x is normalized in order to compare the coefficient among x variables
# ##############
import optuna
import glob
import os
...
# Remove_predictor = [Target_n, 'STFIPS', ]
...
# Set target
p_tr = [Target_n, 'Trip_Rate', 'Trip_Density']
data['Trip_Rate'] = data[Target_n] / (data['Total Population'] * 1e4)
data['Trip_Density'] = data[Target_n] / (data['Area'])
data_unseen['Trip_Rate'] = data_unseen[Target_n] / (data_unseen['Total Population'] * 1e4)
data_unseen['Trip_Density'] = data_unseen[Target_n] / (data_unseen['Area'])
...
# No transfer Y
normalized_train_y = data_train_y.copy()
normalized_test_y = data_test_y.copy()
...
def tuning_result(date_num, objective_f, n_trials, normalized_train_x, normalized_train_y, normalized_test_x,
    ...
    # Predict on testing
    print('MAPE: %s' % mean_absolute_percentage_error(normalized_test_y.values,
...
def objective_lgb_sk(trial, normalized_train_x, normalized_train_y, date_num):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(fold.split(range(len(normalized_train_x)))):
        ...
        accuracy = mean_absolute_percentage_error(normalized_train_y.loc[valid_idx].values,
                                                  gbm.predict(normalized_train_x.loc[valid_idx]).reshape(-1, 1))
...
def objective_mlp(trial, normalized_train_x, normalized_train_y, date_num):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(fold.split(range(len(normalized_train_x)))):
        ...
        accuracy = mean_absolute_percentage_error(normalized_train_y.loc[valid_idx].values,
                                                  mlpr.predict(normalized_train_x.loc[valid_idx]).reshape(-1, 1))
        scores.append(accuracy)
    # Save a trained model
    with open(os.path.join(os.getcwd(), "optuna_" + date_num, "sr_{}-{}.pickle".format(trial.number, np.mean(scores))),
              "wb") as f: pickle.dump(scores, f)
...
def objective_hul(trial, normalized_train_x, normalized_train_y, date_num):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(fold.split(range(len(normalized_train_x)))):
        ...
        accuracy = mean_absolute_percentage_error(normalized_train_y.loc[valid_idx].values,
                                                  huberlinear.predict(normalized_train_x.loc[valid_idx]).reshape(-1, 1))
        scores.append(accuracy)
    # Save a trained model
    with open(os.path.join(os.getcwd(), "optuna_" + date_num, "sr_{}-{}.pickle".format(trial.number, np.mean(scores))),
              "wb") as f: pickle.dump(scores, f)
...
def objective_lasso(trial, normalized_train_x, normalized_train_y, date_num):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(fold.split(range(len(normalized_train_x)))):
        ...
        accuracy = mean_absolute_percentage_error(normalized_train_y.loc[valid_idx].values,
                                                  lassl.predict(normalized_train_x.loc[valid_idx]).reshape(-1, 1))
        scores.append(accuracy)
    # Save a trained model
    with open(os.path.join(os.getcwd(), "optuna_" + date_num, "sr_{}-{}.pickle".format(trial.number, np.mean(scores))),
              "wb") as f: pickle.dump(scores, f)
...
def objective_ridge(trial, normalized_train_x, normalized_train_y, date_num):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(fold.split(range(len(normalized_train_x)))):
        ...
        accuracy = mean_absolute_percentage_error(normalized_train_y.loc[valid_idx].values,
                                                  ridl.predict(normalized_train_x.loc[valid_idx]).reshape(-1, 1))
        scores.append(accuracy)
    # Save a trained model
    with open(os.path.join(os.getcwd(), "optuna_" + date_num, "sr_{}-{}.pickle".format(trial.number, np.mean(scores))),
              "wb") as f: pickle.dump(scores, f)
...
def objective_enet(trial, normalized_train_x, normalized_train_y, date_num):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(fold.split(range(len(normalized_train_x)))):
        ...
        accuracy = mean_absolute_percentage_error(normalized_train_y.loc[valid_idx].values,
                                                  enet.predict(normalized_train_x.loc[valid_idx]).reshape(-1, 1))
        scores.append(accuracy)
    # Save a trained model
    with open(os.path.join(os.getcwd(), "optuna_" + date_num, "sr_{}-{}.pickle".format(trial.number, np.mean(scores))),
              "wb") as f: pickle.dump(scores, f)
...
def objective_llar(trial, normalized_train_x, normalized_train_y, date_num):
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(fold.split(range(len(normalized_train_x)))):
        ...
        accuracy = mean_absolute_percentage_error(normalized_train_y.loc[valid_idx].values,
                                                  llar.predict(normalized_train_x.loc[valid_idx]).reshape(-1, 1))
        scores.append(accuracy)
    # Save a trained model
    with open(os.path.join(os.getcwd(), "optuna_" + date_num, "sr_{}-{}.pickle".format(trial.number, np.mean(scores))),
              "wb") as f: pickle.dump(scores, f)
...
Allresults = []  # 'mlp', 'xgb_sk', 'lgb_sk', 'catb', 'rf', 'etr'
for kk in ['llar', 'enet', 'ridge', 'lasso', 'hul', 'dct', ]:
    number_trails = a_tr
